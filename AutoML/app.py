...
uploaded_file = st.sidebar.file_uploader(
    label="ファイルアップロード", label_visibility="collapsed"
)
...
if select_data in st.session_state["analysis_list"].keys():
    ...
    with set_col3:
        train_size = st.number_input(
            label="学習データの割合", min_value=0.0, max_value=1.0, value=0.7
        )
    with set_col4:
    ...
    with tab3:
        ...
        with set_col1:
            fold_strategy = st.radio(
                label="CVを選択",
                options=["kfold", "groupkfold", "timeseries"],
                horizontal=True,
            )
        ...
        with set_col3:
            fold_shuffle = st.radio(
                label="CVのシャッフルを行うか", options=[True, False], horizontal=True
            )
        ...
        if "pycaret_setup" in st.session_state:
            ...
            with pycaret_tab3:
                ...
                plot_list = plot_model_list.loc[
                    plot_model_list["Modules"] == ml_task, "ID"
                ].values
                col1, col2 = st.columns([2, 1])
                with col1:
                    plot_contents = st.radio(
                        label="可視化項目を選択", options=plot_list, horizontal=True
                    )
                    st.write(
                        plot_model_list.loc[
                            (plot_model_list["Modules"] == ml_task)
                            & (plot_model_list["ID"] == plot_contents),
                            "Name",
                        ].values[0]
                    )
                with col2:
                    try:
                        s.plot_model(
                            best, plot=plot_contents, display_format="streamlit"
                        )
            ...
            with pycaret_tab4:
                ...
                components.iframe(
                    "http://localhost:5000", width=None, height=800, scrolling=True
                )
            with pycaret_tab5:
                ...
                if "Deepchecks" in st.session_state:
                    components.html(
                        st.session_state["Deepchecks"], height=600, scrolling=True
                    )
            with pycaret_tab6:
                # ExplainerDashboardは随時サーバーが立ち上がってしまうので、ボタンクリック時のみ表示する
                # TODO:shutdown実装
                st.write(
                    "ExplainerDashboardを起動してからクリックで別窓表示→[📰ExplainerDashboard](http://localhost:8050/)"
                )
                if st.button("ExplainerDashboardの起動"):
                    components.iframe(
                        "http://localhost:8050", width=None, height=800, scrolling=True
                    )
                    s.dashboard(best)
    ...
    with tab4:
        ...
        with set_col2:
            tabular_or_multimodel = st.radio(
                label="扱うデータ種別を選択", options=["表形式", "マルチモーダル"], horizontal=True
            )
        with set_col3:
            if ml_task == "Classification":
                ml_tasks_autogluon = st.radio(
                    label="分類タスクを選択", options=["binary", "multiclass"], horizontal=True
                )
            if ml_task == "Regression":
        ...
        if st.button(label="計算実行", key="AutoGluon_計算実行"):
            ...
            with st.spinner("AutoGluon実行中…"):
                ...
                train, test = train_test_split(
                    df_data, train_size=train_size, shuffle=True, random_state=0
                )
                train_data = TabularDataset(train)
                if tabular_or_multimodel == "表形式":
                    predictor = TabularPredictor(
                        label=target_col, problem_type=ml_tasks_autogluon
                    ).fit(train_data, presets=select_presets, time_limit=time_limit)
