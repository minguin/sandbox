...
def create_notion_page(result, summary, response_semanticscholar):
    ...
    payload["properties"]["Date"]["date"]["start"] = result.published.strftime(
        "%Y-%m-%d %H:%M:%S"
    )
...
def main():
    ...
    ARXIV_QUERY = st.text_input(
        "クエリを入力", "all:%22 reciprocal %22 AND all:%22 recommend %22"
    )
    st.code(
        """
    - タイトル検索:ti
    ...
    if st.button("計算実行"):
        ...
        for i, s in enumerate(response_semanticscholar):
            if (s is None) | (s == "error"):
                ...
                # 日数あたりの引用回数
                order_list.append(
                    -s["citationCount"] / ((now - results[i].published).days + 1)
